...
int altaPelicula(sPelicula listadoPeliculas[], sActor listadoActores[], int indice, int limite)
{
    ...
    listadoPeliculas[indice].genero.id=pedirEntero("Ingrese el genero:\n1.Accion\n2.Comedia\n3.Drama\n4.Romance\n5.Terror\n");
    listarActores(listadoActores, MAXACTORES);
    listadoPeliculas[indice].actor.id=pedirEntero("Ingrese numero del actor: ");
...
int mostrarPelicula(sPelicula listadoPeliculas[], int indice)
{
    printf("%4d-%29s -%2d/%2d/%4d- %2d - %d\n", listadoPeliculas[indice].id, listadoPeliculas[indice].titulo, listadoPeliculas[indice].fechaEstreno.dia, listadoPeliculas[indice].fechaEstreno.mes,
           listadoPeliculas[indice].fechaEstreno.anio, listadoPeliculas[indice].actor.id, listadoPeliculas[indice].genero.id);
...
int hardcodePeliculas(sPelicula listadoPeliculas[])
{
    ...
    for(i=0; i<21; i++)
    {
        ...
        listadoPeliculas[i].actor.id=actor[i];
        listadoPeliculas[i].genero.id=genero[i];
...
int modificarPelicula(sPelicula listadoPeliculas[], sActor listadoActores[], int limite)
{
    ...
    switch(opcion)
    {
        ...
        case 2:
            ...
            break;
        case 3:
            retorno=modificarActor(listadoPeliculas, listadoActores, indice);
            break;
    }
    return retorno;
}
...
int modificarActor(sPelicula listadoPeliculas[], sActor listadoActores[], int indice)
{
    ...
    {
        listarActores(listadoActores, MAXACTORES);
        listadoPeliculas[indice].actor.id=pedirEntero("Ingrese nuevo actor: ");
        printf("\nModificacion exitosa!\n");
        retorno=0;
    }
    return retorno;
